...
# 2026-08-26 EE sourcing note: riigiteataja.ee (Estonia's official gazette) is
# JS-rendered -- same known blocker as LT's e-seimas.lrs.lt and DA's
# retsinformation.dk (see project-manual-sourcing-backlog memory). WebFetch
# against it returns only a loading shell, no primary text. Every EE entry
# added below (and in _LAW_TRANS, _STUK_REPLACEMENT, _COUNTRY_LUVAT["EE"],
# _COUNTRY_LIITTEET["EE"]) was cross-checked across 2+ independent secondary
# sources per fact instead (Kliimaministeerium, Riigikogu press releases,
# k6k.ee/Keskkonnaõiguse Keskus, indexed Riigi Teataja search snippets) --
# not verified against directly-fetched primary text the way LV's were.
# Treat as a genuinely lower-confidence sourcing tier; revisit with direct
# primary-text verification if riigiteataja.ee ever becomes fetchable
# (same "flag, don't silently upgrade confidence" treatment as DA's
# Patoturvallisuuslaki gap below).
_LAW_CITATION_REPLACEMENT: dict[str, dict[str, str]] = {
    ...
    "YSL":                  {"SE": "Miljöbalken kap. 9 (miljöfarlig verksamhet)",
                              ...
                              # EE: same shape as LV above -- NOT Keskkonnaseadustiku
                              # üldosa seadus (KeÜS), which is the general framework act
                              # (adopted 16.02.2011, RT I 28.02.2011,1 -- confirmed real,
                              # phased in-force through 2017, but only defines the general
                              # permit concept). The actual operative industrial/IPPC
                              # permit-granting law is Tööstusheite seadus (adopted
                              # 24.04.2013, RT I 16.05.2013, in force 01.06.2013,
                              # implements EU IED 2010/75/EU) -- grants kompleksluba
                              # (integrated permit, above IED capacity threshold) or
    ...
    "Vesilaki":             {"SE": "Miljöbalken kap. 11 (vattenverksamhet)",
                              ...
                              # EE: Veeseadus -- a real 2019 recodification (passed
                              # 30.01.2019, in force 01.10.2019), NOT the older 1994
                              # Veeseadus it replaced. A later substantive amendment
                              # (13.09.2021, in force 01.10.2021) is easy to mistake for
    ...
    "Pelastuslaki":         {"SE": "Lag (2003:778) om skydd mot olyckor",
                              ...
                              # EE: Tuleohutuse seadus (adopted 05.05.2010, RT I 2010,
                              # 24, 116, in force 01.09.2010) -- the concept was already
                              # right in _COUNTRY_LUVAT["EE"], just misspelled as one word
                              # ("Tuleohutusseadus") without the genitive space; corrected
                              # there too. NOT Päästeseadus (2010, RT I 2010, 24, 115, a
                              # same-day companion act) -- that law governs the Rescue
                              # Board's rescue-operation powers, not fire-safety technical
                              # requirements/permits. NOT Päästeteenistuse seadus (2022)
    ...
    #  - EE: a fifth, distinct case -- NOT "no framework" (the previous
    #    content) and NOT a fully operative system either. Tuumaenergia ja
    #    -ohutuse seadus (Nuclear Energy and Safety Act, TEOS, bill 856 SE)
    #    passed its THIRD READING in the Riigikogu on 17.06.2026 (63 in
    #    favour, 10 against, 1 abstention) -- a real, completed legislative
    #    event, not a live draft. It is NOT YET IN FORCE as of today
    #    (2026-08-26); entry into force is a firm, already-scheduled date:
    #    01.01.2027. Creates a tiered licensing system (preliminary
    #    assessment -> construction permit -> testing permit -> operating
    #    permit -> decommissioning permit) and an independent nuclear
    #    regulator inside TTJA (Tarbijakaitse ja Tehnilise Järelevalve Amet)
    #    -- TTJA is already a cited EE authority (electrical/technical
    #    equipment safety, Tukes-equivalent); the nuclear regulator is a new
    #    function added to it from 01.01.2027, not a new body. Any decision
    #    to actually BUILD a plant needs separate Riigikogu approval (added
    #    during passage) -- the law creates the licensing framework, not
    #    construction authorisation itself. Realistic commercial timeline is
    #    still "mid-2030s at the earliest" per its own supporting coverage --
    #    same "real framework, no first-of-kind commercial process yet"
    #    shape as LV/NO, just with an adopted-but-not-in-force law instead of
    ...
    "Ydinenergialaki":      {"SE": "Lag (1984:3) om kärnteknisk verksamhet",
                              ...
                              "EE": ("Tuumaenergia ja -ohutuse seadus (TEOS) — Riigikogu võttis vastu "
                                     "17.06.2026 (III lugemine, 63 poolt), kuid seadus EI OLE VEEL JÕUS: "
                                     "jõustub 1. jaanuaril 2027. Loob astmelise loamenetluse (eelhinnang, "
                                     "ehitusluba, katseluba, kasutusluba, dekomisjoneerimisluba) ja uue "
                                     "tuumaregulaatori TTJA (Tarbijakaitse ja Tehnilise Järelevalve Amet) "
    ...
    # for B/C).
    # EE also deliberately has no entry (2026-08-26): dam safety was
    # explicitly out of scope for this research pass (only KeÜS,
...
# 2026-08-26 correction: EE previously had no entry here at all -- SMR/BESS
# generation for EE didn't rely on this backstop before, since
# _COUNTRY_LUVAT["EE"]'s SMR/smr_ee rows already wrote EE-specific text
# directly rather than inheriting a Finnish EN card with "STUK" in it. Added
# proactively in case any future EE card falls back to the generic EN
# template. Deliberately does NOT say TTJA "is" the nuclear regulator today
# -- TEOS (Tuumaenergia ja -ohutuse seadus) was adopted 17.06.2026 but only
# enters into force 01.01.2027; see the Ydinenergialaki comment above for the
# full status. Sourced from cross-checked secondary sources, not fetched
# primary text (riigiteataja.ee is JS-rendered) -- see the sourcing-tier note
# above _LAW_CITATION_REPLACEMENT.
...
_HANKE_CFG = {
    "BESS": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: Kaavoitus- ja rakennuslupavaatimus vastaa Plan- och "
            "bygglagen (PBL, 2010:900) mukaista bygglov/detaljplan-menettelyä; PBL "
            "kattaa sekä kaavoituksen että rakennusluvan samassa laissa, toisin kuin "
            "Suomen erillinen MRL/Rakentamislaki-jako.\n"
            "- LV-erityinen: Kaavoitusvaatimus on detālplānojums, tarvittaessa "
            "[Teritorijas attīstības plānošanas likums (2011)]. Rakennuslupa "
            "(būvatļauja) haetaan pašvaldības būvvaldelta digitaalisesti BIS-portaalin "
            "kautta (bis.gov.lv) — vaatimukset vaihtelevat kunnittain "
            "[Būvniecības likums (2013)].\n"
            ...
            "- SE-erityinen: Ympäristölupa (miljöfarlig verksamhet) käsitellään "
            "Miljöbalken kap. 9 mukaisesti; tarkkaa pohjavesialue- tai "
            "IED-kynnysarvoa ei ole vahvistettu Ruotsille [Vaatii tarkistuksen].\n"
            "- LV-erityinen: Piesārņojošas darbības atļauja tai ilmoitus koon mukaan "
            "[Likums \"Par piesārņojumu\" (2001)] — A-kategorian laitokset on lueteltu "
            "lain omassa liitteessä 1, B/C-kategoriat MK noteikumi Nr. 1082:n "
            "liitteissä; luvan myöntää Valsts vides dienests (VVD). HUOM: ei "
            "Vides aizsardzības likums (2006, yleinen kehyslaki) — se ei ole "
            "operatiivinen lupalaki.\n"
            ...
            "- SE-erityinen: Verkkoliityntä hoidetaan Svenska kraftnätin kautta "
            "suurille laitoksille tai paikallisen nätägarens kautta pienemmille.\n"
            "- DE-erityinen: Hankkeen omistaja voi Saksassa itse valita, hakeeko "
            "verkkoliitynnän Übertragungsnetzbetreiberiltä (ÜNB — neljä valtakunnallista "
            "kantaverkkoyhtiötä) vai paikalliselta Verteilnetzbetreiberiltä (VNB) — valinta ei "
            "ole automaattinen tehon perusteella kuten Suomen SJV/VJV-jaossa. Huomio: "
            "ÜNB-tason liittymiskapasiteetti oli vuoden 2026 alussa raportoitu paikoin "
            "täyteen varattuna suurten akkuvarastojen kysyntäpiikin vuoksi — tilanne "
            "voi vaikuttaa liittymisen käsittelyaikaan [Vaatii tarkistuksen ajantasaisen "
            "kapasiteettitilanteen osalta]. Erillinen, ajankohtainen sääntelykysymys: "
            "akkuvarasto luokitellaan verkosta ladattaessa kuluttajaksi ja purkaessa "
            "tuottajaksi, mikä on aiemmin johtanut kaksinkertaiseen verkkomaksu-/EEG-"
            "maksurasitukseen (Doppelbelastung); EEG:n § 61l poistaa kaksinkertaisen "
            "EEG-maksun, ja EnWG-uudistus (2024, § 3 Nr. 15d EnWG) määrittelee "
            "sähkövarastot omaksi kategoriakseen — ei kuluttajaksi eikä tuottajaksi — "
            "mikä poistaa myös muun kaksinkertaisen maksuvelvoitteen.\n"
            ...
            "- SE-erityinen: Ruotsin vastaava sähköturvallisuusviranomainen on "
            "Elsäkerhetsverket; tarkkaa tarkastusprosessia ja säädösviittausta ei "
            "ole vahvistettu [Vaatii tarkistuksen].\n"
            ...
            "- SE-erityinen: Palo- ja räjähdysturvallisuutta akkuvarastoille säätelee "
            "Lag (2010:1011) om brandfarliga och explosiva varor (LBE), jota valvoo "
            "MSB (Myndigheten för samhällsskydd och beredskap); Tukes S10-ohjetta "
            "vastaavaa ruotsalaista teknistä ohjeistusta ei ole vahvistettu "
            "[Vaatii tarkistuksen].\n"
            "- LV-erityinen: Paloturvallisuuslausunto (ugunsdrošības atzinums) on "
            "pakollinen BESS-hankkeille [Ugunsdrošības, ugunsdzēsības un glābšanas "
            "darbu likums, voimassa 13.11.2025 alkaen — HUOM: vanha vuoden 2002 laki "
            "samalla nimellä on kumottu, älä viittaa siihen] — myöntää VUGD (Valsts "
            "ugunsdzēsības un glābšanas dienests). Tukes S10-ohjetta vastaavaa "
            "latvialaista teknistä ohjeistusta ei ole tunnistettu "
            "[Vaatii tarkistuksen].\n"
            "- DE-erityinen: Poiketen useimmista muista tässä käsitellyistä maista, "
            "itsenäinen akkuvarasto EI vaadi Saksassa BImSchG-ympäristölupaa — "
            "akkuvarastoja ei ole listattu 4. BImSchV:n Liite 1 -luettelossa, joten "
            "vaadittava lupa on tavallinen rakennuslupa (Baugenehmigung). Ulkoalueella "
            "(Außenbereich) rakennuslupa vaaditaan lähes aina; sisäalueella "
            "(Innenbereich) vaatimus vaihtelee osavaltiokohtaisesti. Paloturvallisuus "
            "ei perustu erilliseen valtakunnalliseen BESS-säädökseen vaan osavaltion "
            "omaan rakennusjärjestykseen (Landesbauordnung) — Tukes S10-ohjetta "
            "vastaavaa yhtenäistä kansallista teknistä ohjeistusta ei ole "
            "[Vaatii tarkistuksen hankkeen osavaltion ja alue-/sisäalueluokituksen osalta].\n"
            ...
            "- SE-erityinen: Pelastussuunnitelma hyväksytetään paikallisella "
            "räddningstjänstillä Lag (2003:778) om skydd mot olyckor -lain nojalla.\n"
            "- LV-erityinen: Verkkoliityntä AST:n (Augstsprieguma tīkls) tai Sadales "
            "tīkls AS:n kautta koon mukaan [Elektroenerģijas tirgus likums §§73-83; "
            "RfG 2016/631 + DCC 2016/1388]. SPRK (Sabiedrisko pakalpojumu "
            "regulēšanas komisija) vaatii rekisteröinnin 50 kW–1 MW hankkeille ja "
            "ražošanas licence -tuotantoluvan yli 1 MW hankkeille [MK noteikumi "
            "Nr. 631]. Paloturvallisuuslausunto haetaan VUGD:ltä ennen käyttöönottoa."
        ),
    ...
    "tuulivoima_maa": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: Ruotsin tuulivoimamelun raja-arvot poikkeavat todennäköisesti "
            "yllä olevista suomalaisista VNa 993/1992 -arvoista — tarkka säädös ja "
            "raja-arvot [Vaatii tarkistuksen].\n"
            "- LV-erityinen: minimietäisyys lähimpään yksittäistalouteen on 800 m; tällä "
            "etäisyydellä melu ei ylitä Latvian raja-arvoja. 2 km säteellä yöllinen melu "
            "vaihtelee 30–42 dB(A) välillä etäisyydestä riippuen — löydetty toimittajien "
            "selvityksistä, ei tarkkaa asetusviittausta vielä vahvistettu "
            "[Vaatii tarkistuksen — tarkka MK-noteikumi-viittaus].\n"
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- LV-erityinen: IVN (ietekmes uz vidi novērtējums) on täysimittaisena pakollinen "
            "hankkeille, joissa on yli 2 voimalaa tai kokonaisteho ylittää 5 MW "
            "[Likums \"Par ietekmes uz vidi novērtējumu\"] — vastaa toiminnallisesti Suomen "
            "YVA-kynnystä, mutta kynnysarvo on matalampi.\n"
            "- SE-erityinen: YVA-harkinta (miljöbedömning) tehdään Miljöbalken kap. 6 "
            "mukaisesti; tarkkaa tuulivoimahankkeiden kynnysarvoa (voimaloiden lukumäärä "
            "tai napakorkeus) ei ole vahvistettu [Vaatii tarkistuksen]. Ympäristölupa "
            "(miljöfarlig verksamhet) käsitellään Länsstyrelsenin "
            "Miljöprövningsdelegationenissa (MPD) Miljöbalken kap. 9 mukaisesti.\n"
            ...
            "- SE-erityinen: Natura 2000 -aluearviointi vaaditaan myös Ruotsissa EU-lainsäädännön "
            "nojalla; tarkka kansallinen säädösviittaus [Vaatii tarkistuksen].\n"
            ...
            "- SE-erityinen: Ympäristölupamenettely (miljöfarlig verksamhet, Miljöbalken kap. 9) "
            "käsitellään Länsstyrelsenin Miljöprövningsdelegationenissa (MPD); julkisen "
            "kuulemisen kesto [Vaatii tarkistuksen]. TÄRKEÄÄ: Ruotsissa sijaintikunnalla on "
            "ehdoton veto-oikeus tuulivoimahankkeisiin (Miljöbalken 16 kap. 4 §) — kunta voi "
            "hylätä hankkeen ilman perustelua, eikä päätökseen voi hakea muutosta; tällä ei "
            "ole vastinetta muissa tässä käsitellyissä maissa, ja se vaikuttaa merkittävästi "
            "hyväksymistodennäköisyyteen (kunnat hylkäsivät n. 75 % maalle sijoitettavista "
            "tuulivoimahankkeista vuoden 2024 alkupuoliskolla).\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- LV-erityinen: Kaavoitusta vastaava vaatimus on detālplānojums "
            "[Teritorijas attīstības plānošanas likums (2011)], pašvaldības domen "
            "hyväksymä. TÄRKEÄÄ: Latvialla ei toistaiseksi ole Ruotsin kaltaista ehdotonta "
            "kunnan veto-oikeutta, mutta uudistus on valmisteilla — pašvaldīballe "
            "ollaan antamassa oikeus kieltäytyä hyväksymästä uutta tuulivoimahanketta, "
            "jos kunnan alueella on jo vähintään 150 MW tai suunnittelualueella "
            "(plānošanas reģions) yhteensä 500 MW tuulivoimaa käytössä — KEM/VARAM:n "
            "ilmoittama uudistus, ei vielä vahvistettu laiksi "
            "[Vaatii tarkistuksen — seurattava lainsäädäntötilannetta].\n"
            "- DE-erityinen: Saksalla ei ole Ruotsin kaltaista suoraa kunnan veto-oikeutta, "
            "mutta osavaltiotason etäisyyssäännöt voivat vaikuttaa käytännössä samansuuntaisesti "
            "— esim. Baijerin ns. 10H-sääntö (Bayerische Bauordnung, voimassa 2014 alkaen) "
            "edellyttää vähintään 10 × voimalan korkeuden etäisyyttä asutukseen (n. 2 km "
            "tyypilliselle 200 m voimalalle); marraskuusta 2022 poikkeuksena kiinteä 1 000 m "
            "metsäalueilla, teollisuusalueiden lähellä sekä osoitetuilla tuulivoima-alueilla. "
            "Sääntö koskee vain Baijeria — muissa osavaltioissa vastaavaa ei ole "
            "[Vaatii tarkistuksen hankkeen osavaltion osalta].\n"
            "- DE-erityinen: Lajisto- ja luonnonsuojelu (Artenschutz) on Saksan tuulivoimalupien "
            "yleisin oikeudellinen kompastuskivi — Bundesnaturschutzgesetzin (BNatSchG) § 44 "
            "kieltää suojeltujen lajien häirinnän/tappamisen, ja rikkomukset ovat johtaneet "
            "toistuviin valituksiin (mm. BVerwG:n ratkaisu 19.12.2023 jälkikäteisistä "
            "käyttörajoituksista). WindBG § 6 kuitenkin keventää arviointia hankkeille, jotka "
            "sijoittuvat viranomaisen osoittamille tuulivoima-alueille (Windenergiegebiete) — "
            "arviointimenettely riippuu siis sijainnista "
            "[Vaatii tarkistuksen osavaltiotason soveltamiskäytännön osalta].\n"
            "- SE-erityinen: Kaavoitusta vastaava vaatimus Ruotsissa on Plan- och bygglagen "
            "(PBL, 2010:900) mukainen detaljplan ja/tai bygglov; PBL kattaa sekä "
            "kaavoituksen että rakennusluvan samassa laissa, toisin kuin Suomen erillinen "
            "MRL/Rakentamislaki-jako.\n"
            ...
            "- SE-erityinen: Svenska kraftnät vastaa kantaverkkoliitynnästä; "
            "liittymisprosessin tarkka kesto [Vaatii tarkistuksen].\n"
            "- LV-erityinen: AST (Augstsprieguma tīkls) vastaa kantaverkkoliitynnästä "
            "[Elektroenerģijas tirgus likums §§73-83; RfG 2016/631]. SPRK "
            "(Sabiedrisko pakalpojumu regulēšanas komisija) myöntää ražošanas licence "
            "-tuotantoluvan hankkeille yli 1 MW [MK noteikumi Nr. 631]. Būvatļauja "
            "(rakennuslupa) haetaan pašvaldības būvvaldelta — vaatimukset vaihtelevat "
            "kunnittain [Būvniecības likums (2013)].\n\n"
            ...
            "- SE-erityinen: Kokonaisaikataulu (miljöprövning + kaavoitus + rakennuslupa) "
            "[Vaatii tarkistuksen] — sijaintikunnan veto-oikeus (ks. KERROS 3) voi vaikuttaa "
            "merkittävästi sekä aikatauluun että lopputulokseen.\n"
            "- LV-erityinen: Kokonaisaikataulu [Vaatii tarkistuksen — ei löydetty "
            "vahvistettua kokonaisarviota] — huomioitava valmisteilla oleva "
            "pašvaldības-kieltäytymisoikeus (ks. KERROS 3), joka voi vaikuttaa "
            "hyväksymistodennäköisyyteen alueilla, joilla on jo paljon tuulivoimaa.\n"
            ...
            "- SE-erityinen: Svenska kraftnätin liittymispisteselvityksen tarkka käsittelyaika "
            "[Vaatii tarkistuksen].\n"
            "- LV-erityinen: AST:n liittymispisteselvityksen tarkka käsittelyaika "
            "[Vaatii tarkistuksen].\n"
    ...
    "tuulivoima_meri": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: Merituulivoimahankkeen YVA (miljöbedömning) tehdään Miljöbalken "
            "kap. 6 mukaisesti; tarkkaa merituulivoiman YVA-kynnystä ei ole vahvistettu "
            "[Vaatii tarkistuksen].\n"
            "- LV-erityinen: IVN [Likums \"Par ietekmes uz vidi novērtējumu\"] koskee myös "
            "merituulivoimaa, mutta tarkkaa offshore-lupamenettelyä (yhden vai useamman "
            "viranomaisen prosessi, tarkka kynnysarvo) ei ole vielä täysin varmistettu "
            "[Vaatii tarkistuksen — Latvian merituulivoimaprosessi kesken tämän tutkimuksen "
            "aikana].\n"
            "- Merialuesuunnitelma: tarkista onko alue osoitettu tuulivoima-alueeksi "
            "[Laki merialuesuunnittelusta 905/2016].\n"
            "- SE-erityinen: Ruotsissa merialuesuunnittelusta vastaa Havs- och "
            "vattenmyndigheten (HaV); tarkka säädösviittaus [Vaatii tarkistuksen].\n"
            "- LV-erityinen: Latvialla on oma merialuesuunnitelma (Latvijas jūras "
            "telpiskais plānojums), mutta tuulivoima-alueiden tarkka osoitus ja vastaava "
            "viranomainen eivät ole vielä täysin varmistettuja [Vaatii tarkistuksen].\n"
            "- DE-erityinen: Saksan merituulivoiman lupa- ja aluejärjestelmä perustuu "
            "Windenergie-auf-See-Gesetziin (WindSeeG). BSH (Bundesamt für Seeschifffahrt und "
            "Hydrographie) toimii varsinaisena lupa- ja valvontaviranomaisena talousvyöhykkeellä "
            "(AWZ) Pohjanmerellä ja Itämerellä ja tekee Planfeststellungsbeschluss-päätöksen "
            "18 kuukauden kuluessa täydellisten suunnitteluasiakirjojen vastaanottamisesta. "
            "Aluekilpailutuksesta (Ausschreibung) vastaa erikseen Bundesnetzagentur (BNetzA) — "
            "kaksi eri viranomaista, joita ei pidä sekoittaa: BSH ympäristö-/turvallisuusluvasta, "
            "BNetzA huutokaupasta [Vaatii tarkistuksen ajantasaisen aluesuunnitelman osalta].\n"
            "- Puolustusvoimien tutkavaikutusarviointi: tehtävä ennen lupaprosessin aloittamista.\n"
            "- SE-erityinen: Ruotsin vastine on Försvarsmakten; tutkavaikutuslausunnon "
            "tarkka prosessi [Vaatii tarkistuksen].\n"
            ...
            "- SE-erityinen: Vastaava vesilupa (tillstånd för vattenverksamhet) käsitellään "
            "Miljöbalken kap. 11 mukaisesti mark- och miljödomstolissa. TÄRKEÄÄ: sijaintikunnan "
            "ehdoton veto-oikeus (Miljöbalken 16 kap. 4 §) koskee myös rannikonläheistä "
            "merituulivoimaa, ei vain maalle sijoitettavaa — sama huomio kuin "
            "tuulivoima_maa-hanketyypissä.\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Merialueen käyttöoikeudesta vastaava viranomainen — vastine "
            "Metsähallitukselle ei ole vahvistettu [Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Svenska kraftnät vastaa kantaverkkoliitynnästä; "
            "liittymisprosessin tarkka kesto [Vaatii tarkistuksen].\n"
            "- LV-erityinen: AST (Augstsprieguma tīkls) vastaa kantaverkkoliitynnästä myös "
            "merituulivoimalle [Elektroenerģijas tirgus likums §§73-83; Komisijas regula (ES) "
            "2016/631 (RfG) / HVDC 2016/1447]. Valtion merialueen käyttöoikeussopimuksesta "
            "vastaava viranomainen (Metsähallituksen vastine) ei ole vielä täysin "
            "varmistettu [Vaatii tarkistuksen].\n\n"

            "KERROS 4 — VIRANOMAISLIITE [aikataulut, kontaktit]:\n"
            "Käytä osiossa '3. Luvat' ja '6. Seuraavat toimenpiteet'.\n"
            "- Kokonaisaikataulu 7–12 vuotta hakemuksesta käyttöönottoon.\n"
            "- SE-erityinen: Kokonaisaikataulu [Vaatii tarkistuksen] — sijaintikunnan "
            "veto-oikeus (ks. KERROS 3) voi vaikuttaa merkittävästi.\n"
            "- LV-erityinen: Kokonaisaikataulu [Vaatii tarkistuksen] — Latvian "
            "merituulivoimaprosessi on vähemmän vakiintunut kuin maalle sijoitettavan "
            "tuulivoiman (ks. KERROS 2/3), mikä todennäköisesti pidentää aikataulua "
            "verrattuna maahankkeisiin.\n"
    ...
    "aurinkovoima": {
        ...
        "context_extra": (
            ...
            "- LV-erityinen: IVN-harkinta (Sākotnējā izvērtēšana) käynnistyy samalla "
            "50 ha:n kynnysarvolla kuin Suomessa [Likums \"Par ietekmes uz vidi "
            "novērtējumu\"] — VPVB (Vides pārraudzības valsts birojs) tekee harkinnan.\n"
            ...
            "- SE-erityinen: Vastaava vaatimus Ruotsissa on bygglov ja/tai detaljplan Plan- "
            "och bygglagen (PBL, 2010:900) mukaisesti; PBL kattaa sekä kaavoituksen että "
            "rakennusluvan samassa laissa, toisin kuin Suomen erillinen MRL/Rakentamislaki-jako.\n"
            "- Ympäristölupa: tarvitaan ≥1 ha tai pohjavesialueelle [YSL 527/2014].\n"
            "- SE-erityinen: Ympäristölupa (miljöfarlig verksamhet) käsitellään Miljöbalken "
            "kap. 9 mukaisesti; tarkkaa hehtaarikynnystä ei ole vahvistettu Ruotsille "
            "[Vaatii tarkistuksen].\n"
            "- Maisema- tai kulttuuriympäristölausunto: ELY-keskus, 30 pv.\n"
            "- SE-erityinen: Maisema- ja kulttuuriympäristöarvioinnista vastaa Länsstyrelsen; "
            "lausunnon tarkka käsittelyaika [Vaatii tarkistuksen].\n"
            "- Verkkoliityntäsopimus: jakeluverkkoyhtiö (DSO), käsittelyaika 3–6 kk.\n"
            "- SE-erityinen: Verkkoliityntä hoidetaan paikallisen nätägarenin (DSO) kautta, "
            "tai suurempien laitosten osalta Svenska kraftnätin kautta; tarkka käsittelyaika "
            "[Vaatii tarkistuksen].\n"
            "- LV-erityinen: Kaavoitusta vastaava vaatimus on detālplānojums, tarvittaessa "
            "[Teritorijas attīstības plānošanas likums (2011)]. Rakennuslupa (būvatļauja) "
            "haetaan pašvaldības būvvaldelta — vaatimukset vaihtelevat kunnittain "
            "[Būvniecības likums (2013)]. Verkkoliityntä Sadales tīkls AS:n kautta.\n"
            ...
            "- SE-erityinen: Bygglov-käsittelyn tarkka kesto [Vaatii tarkistuksen].\n"
            "- ELY-keskus maisema: lausunto 30 pv; Natura-arviointi lisää 3–6 kk.\n"
            "- SE-erityinen: Länsstyrelsenin lausunnon ja mahdollisen Natura-arvioinnin "
            "tarkka kesto [Vaatii tarkistuksen].\n"
    ...
    "SMR": {
        ...
        "context_extra": (
            ...
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            "'STUK YVL-ohje': STUK:n YVL-ohjeisto on sitova sääntely. ÄLÄ listaa YVL A.1, "
            "B.1 ja C.1 pelkkänä luettelona 'kaikki soveltuvat' -toteamuksella — viittaa "
            "jokaiseen erikseen ja kirjoita YKSI lyhyt, konkreettinen perustelu jokaiselle "
            "siitä, miten hanke täyttää/arvioi kyseisen ohjeen vaatimuksen, perustuen "
            "haetun lähdeaineiston (RAG-kontekstin) todelliseen sisältöön: "
            "YVL A.1 (turvallisuusvaatimukset — esim. defence-in-depth-periaatteen "
            "toteutus tässä hankkeessa), YVL B.1 (rakenteelliset perusteet — esim. "
            "suunnitteluperusteonnettomuuksien kattavuus), YVL C.1 (säteilyturvallisuus-"
            "analyysi — esim. annosrajojen arviointimenetelmä). Jos haettu lähdeaineisto "
            "ei riitä konkreettisen perustelun kirjoittamiseen jollekin näistä kolmesta, "
            "merkitse se rehellisesti [Vaatii tarkistuksen] äläkä keksi yksityiskohtia. "
            "Muita YVL-ohjeita (STUK:n koko 46 ohjeen sarjasta vain nämä kolme on "
            "sisällytetty tähän tietokantaan) ei saa mainita tässä kohdassa ikään kuin "
            "niistäkin olisi sisältöä käytettävissä.\n"
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: SSM (Strålsäkerhetsmyndigheten) on Ruotsin ydin- ja "
            "säteilyturvallisuusviranomainen; SSM:n omat turvallisuusmääräykset (SSMFS) "
            "vastaavat STUKin YVL-ohjeistoa, mutta tarkkaa SSMFS-numerointia ei ole "
            "vahvistettu [Vaatii tarkistuksen]. IAEA SSR-2/1 -standardi koskee myös "
            "Ruotsia kansainvälisenä turvallisuusstandardina.\n"
            ...
            "- LV-erityinen: Latvialla ei ole kaupallista ydinvoimalaa eikä STUKia "
            "vastaavaa erillistä ydinturvallisuusviranomaista, mutta säteilyturvallisuutta "
            "sääntelee todellinen laki [Likums \"Par radiācijas drošību un kodoldrošību\", "
            "2000, muutettu] ja sitä valvoo Radiācijas drošības centrs (Valsts vides "
            "dienesta alaisuudessa). Latvia osallistuu todelliseen ASV-Latvijas FIRST "
            "-hankkeeseen (Foundational Infrastructure for Responsible Use of SMR "
            "Technology) SMR-valmiuden kartoittamiseksi — ei fiktiivinen, todellinen "
            "yhdysvaltalais-latvialainen ohjelma.\n\n"
            ...
            "- SE-erityinen: Vastaava sääntely on Lag (1984:3) om kärnteknisk verksamhet, "
            "jota valvoo SSM. YVA (miljöbedömning) tehdään Miljöbalken kap. 6 mukaisesti "
            "— ydinlaitoksille YVA on aina pakollinen samoin kuin Suomessa. Ruotsissa on "
            "toisin kuin useissa muissa tässä käsitellyissä maissa aktiivisia toimivia "
            "ydinvoimaloita (Ringhals, Forsmark, Oskarshamn).\n"
            ...
            "- EE-erityinen: Tuumaenergia ja -ohutuse seadus (TEOS) — Riigikogu hyväksyi "
            "17.06.2026, mutta laki EI OLE VIELÄ VOIMASSA: voimaantulo 1.1.2027. Ei "
            "esitetä tätä puuttuvana lakina eikä voimassa olevana järjestelmänä — "
            "tarkka tila on 'hyväksytty, voimaan 1.1.2027'.\n"
            "- DE-erityinen: Atomgesetz (AtG) KIELTÄÄ uusien kaupallisten ydinvoimaloiden "
            "rakentamisen sähköntuotantoon — Saksa toteutti ydinvoimasta luopumisen "
            "15.4.2023. Ei tavallinen, vaikea lupapolku — nimenomainen kielto, "
            "vastaava vakavuusaste kuin Tanskan kiellolla.\n"
            "- LV-erityinen: ks. yllä KERROS 1 — Latvialla ei ole kaupallista "
            "ydinvoimalaa, mutta todellinen säteilyturvallisuuslaki ja -viranomainen "
            "sekä aito nykyinen kiinnostus (FIRST-hanke) ovat olemassa.\n"
            "Muissa maissa: kansallinen ydinenergiasäädös soveltuu.\n\n"
            ...
            "- SE-erityinen: Ruotsin vastine periaatepäätökselle on hallituksen "
            "(regeringen) tillståndsprövning Lag (1984:3) om kärnteknisk verksamhet "
            "-lain nojalla — päätöksen tekee valtioneuvosto (regeringen), ei erillinen "
            "ydinturvallisuusviranomainen. SSM:n turvallisuustarkastus sekä "
            "rakentamis- ja käyttölupa vastaavat STUKin rakentamis- ja "
            "käyttölupaprosesseja, mutta tarkkoja lupavaiheiden pykäläviittauksia "
            "ei ole vahvistettu [Vaatii tarkistuksen].\n"
            "- LV-erityinen: Vakiintunutta periaatepäätös- tai rakentamis-/käyttölupa-"
            "menettelyä ydinlaitokselle ei ole olemassa, koska hanketta ei ole vielä "
            "esitetty. Todellinen sääntelijä (Radiācijas drošības centrs) valvoo "
            "nykyisin säteilyn käyttöä yleisesti, ei reaktorien rakentamislupia "
            "erikseen [Vaatii tarkistuksen, jos hanke etenee todelliseksi]. FIRST-"
            "hankkeen mukaan valmiuden arvioidaan kestävän n. 15 vuotta.\n\n"
            ...
            "- SE-erityinen: Kokonaisaikataulu (hallituksen tillståndsprövning + SSM:n "
            "turvallisuusarviointi + rakentamis- ja käyttölupa) [Vaatii tarkistuksen] "
            "— Ruotsissa on vakiintunut, toimiva ydinvoimaprosessi olemassa olevien "
            "laitosten osalta, mutta uuden SMR-hankkeen tarkkaa kokonaisaikataulua "
            "ei ole vahvistettu.\n"
            "- LV-erityinen: Kokonaisaikataulu ei ole arvioitavissa vakiintuneen "
            "prosessin puuttuessa; FIRST-hankkeen omien lausuntojen mukaan täysi "
            "valmius voisi kestää n. 15 vuotta sääntelykehyksen kehittämisen vuoksi "
            "[Vaatii tarkistuksen]."
        ),
    ...
    "vesivoima": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: Padon turvallisuusluokitus ja valvonta perustuu "
            "Förordning (2014:214) om dammsäkerhet -asetukseen, jota valvoo "
            "Länsstyrelsen; tarkkaa vastaavuutta P1/P2/P3-luokkiin ei ole "
            "vahvistettu [Vaatii tarkistuksen].\n"
            "- LV-erityinen: Latvialla on oma, erillinen, edelleen voimassa oleva "
            "patoturvallisuuslaki [Likums \"Par hidroelektrostaciju hidrotehnisko "
            "būvju drošumu\", 2000/2001] — toisin kuin useissa muissa tässä "
            "käsitellyissä maissa, joissa erillistä patoturvallisuuslakia ei ole. "
            "A-luokan padoille valvonta kuuluu BVKB:lle (Būvniecības valsts "
            "kontroles birojs), B/C-luokille pašvaldības (kunnan) komissioille.\n"
            "- DE-erityinen: Saksalla ei ole Latvian kaltaista erillistä valtakunnallista "
            "patoturvallisuuslakia — perusta on yleinen vesilaki Wasserhaushaltsgesetz (WHG) "
            "yhdistettynä osavaltiokohtaisiin Landeswassergesetzeihin sekä nimenomaiseen "
            "tekniseen standardisarjaan DIN 19700 (\"Stauanlagen\"), jonka osa 11 koskee "
            "nimenomaan Talsperren (patoja/tekojärviä). Rakenne muistuttaa siis enemmän "
            "Puolan/Liettuan mallia (ei erillistä lakia) kuin Ruotsin/Latvian mallia (erillinen "
            "laki/asetus), mutta poikkeaa molemmista siinä, että sovellettava tekninen "
            "standardi on nimetty ja tarkka [Vaatii tarkistuksen osavaltiokohtaisen "
            "valvontaviranomaisen osalta].\n"
            ...
            "- SE-erityinen: YVA (miljöbedömning) tehdään Miljöbalken kap. 6 mukaisesti; "
            "tarkkaa vesivoimalan YVA-kynnystä (teho tai altaan pinta-ala) ei ole "
            "vahvistettu Ruotsille [Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '2. Perustelut' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Vastaava vesilupa (tillstånd för vattenverksamhet) "
            "käsitellään Miljöbalken kap. 11 mukaisesti mark- och miljödomstolissa; "
            "käsittelyajan tarkka kesto [Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Padon vahingonvaaraselvitys ja hätätilanneohjeet "
            "toimitetaan Länsstyrelsenille Förordning (2014:214) om dammsäkerhet "
            "-asetuksen nojalla.\n"
            ...
            "- LV-erityinen: Vesilupaa vastaava lupa on ūdens resursu izmantošanas "
            "atļauja [Ūdens apsaimniekošanas likums (2002)]. A-luokan padon "
            "turvallisuusasiakirjat toimitetaan BVKB:lle, B/C-luokan pašvaldības "
            "komissiolle [ks. KERROS 1].\n\n"

            "KERROS 4 — VIRANOMAISLIITE [aikataulut, kontaktit]:\n"
            "Käytä osiossa '3. Luvat' ja '6. Seuraavat toimenpiteet'.\n"
            "- Luova vesilupa: 2–5 vuotta käsittelyaika — pisin yksittäinen lupaprosessi.\n"
            "- SE-erityinen: Kokonaisaikataulu (miljöprövning + "
            "vattenverksamhet-lupa) [Vaatii tarkistuksen].\n"
    ...
    "hybridi": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: BESS-osan palo- ja räjähdysturvallisuutta säätelee Lag "
            "(2010:1011) om brandfarliga och explosiva varor (LBE), jota valvoo MSB; "
            "Tukes S10-ohjetta vastaavaa ruotsalaista teknistä ohjeistusta ei ole "
            "vahvistettu [Vaatii tarkistuksen].\n"
            "- LV-erityinen: BESS-osan paloturvallisuuslausunto (ugunsdrošības "
            "atzinums) myöntää VUGD [Ugunsdrošības, ugunsdzēsības un glābšanas "
            "darbu likums, voimassa 13.11.2025 alkaen].\n"
            "- Tuulivoimalakomponentti: napakorkeusarvio, melumalli IEC 61400-11, "
            "varjostusvaikutusanalyysi.\n"
            "- DE-erityinen: EEG 2023 mahdollistaa hybridihankkeille oman "
            "tukimuodon, Innovationsausschreibung — yhdistetty tarjouskilpailu, jossa "
            "tuuli- ja/tai aurinkovoima yhdistetään varastoon samassa "
            "liittymispisteessä; edellytyksenä vähintään yksi tuuli-/aurinkokomponentti "
            "ja yhteisteho vähintään 1 MW. Bundesnetzagentur asetti "
            "kattohinnaksi 9,18 snt/kWh (joulukuun 2022 kilpailutuksessa) — "
            "korkeampi kuin tavanomaisissa erillisissä tarjouskilpailuissa, koska "
            "yhdistetty ratkaisu tuottaa lisäarvoa verkolle. Reaalinen esimerkki: "
            "Abo Windin PV+varasto-hanke Lahrissa (Rheinland-Pfalz) hyväksyttiin "
            "ensimmäisellä Innovationsausschreibung-kierroksella syksyllä 2020 "
            "[Vaatii tarkistuksen ajantasaisen kattohinnan ja kilpailutusaikataulun "
            "osalta — muuttuu kilpailutuskierroksittain].\n\n"
            ...
            "- SE-erityinen: YVA (miljöbedömning) tehdään Miljöbalken kap. 6 mukaisesti; "
            "tarkkaa hybridihankkeen YVA-kynnystä ei ole vahvistettu Ruotsille "
            "[Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '2. Perustelut' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: BESS-komponentin ympäristölupa (miljöfarlig verksamhet) "
            "käsitellään Miljöbalken kap. 9 mukaisesti.\n"
            "- LV-erityinen: BESS-komponentin piesārņojošas darbības atļauja tai "
            "ilmoitus [Likums \"Par piesārņojumu\" (2001)] — VVD myöntää; IVN "
            "tuulivoimakomponentin osalta [Likums \"Par ietekmes uz vidi "
            "novērtējumu\"].\n"
            ...
            "- SE-erityinen: Kaavoitusta vastaava vaatimus Ruotsissa on Plan- och "
            "bygglagen (PBL, 2010:900) mukainen detaljplan ja/tai bygglov, joka kattaa "
            "sekä kaavoituksen että rakennusluvan. TÄRKEÄÄ: tuulivoimakomponentin "
            "vuoksi sijaintikunnalla on ehdoton veto-oikeus (Miljöbalken 16 kap. 4 §) "
            "— sama huomio kuin tuulivoima_maa-hanketyypissä.\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Svenska kraftnät vastaa kantaverkkoliitynnästä suurille "
            "yhdistetyille laitoksille.\n"
            "- LV-erityinen: Kaavoitusvaatimus on detālplānojums [Teritorijas "
            "attīstības plānošanas likums (2011)]; verkkoliityntä AST:n tai Sadales "
            "tīkls AS:n kautta koon mukaan [Elektroenerģijas tirgus likums §§73-83]; "
            "rakennuslupa (būvatļauja) pašvaldības būvvaldelta [Būvniecības likums "
            "(2013)].\n\n"
    ...
    "smr_bess": {
        ...
        "context_extra": (
            ...
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            "'STUK YVL-ohje': STUK:n YVL-ohjeisto on sitova sääntely SMR-komponentille. "
            "ÄLÄ listaa YVL A.1, B.1 ja C.1 pelkkänä luettelona 'kaikki soveltuvat' "
            "-toteamuksella — viittaa jokaiseen erikseen ja kirjoita YKSI lyhyt, "
            "konkreettinen perustelu jokaiselle siitä, miten hanke täyttää/arvioi "
            "kyseisen ohjeen vaatimuksen, perustuen haetun lähdeaineiston (RAG-"
            "kontekstin) todelliseen sisältöön: YVL A.1 (turvallisuusvaatimukset), "
            "YVL B.1 (rakenteelliset perusteet), YVL C.1 (säteilyturvallisuusanalyysi). "
            "Jos haettu lähdeaineisto ei riitä konkreettisen perustelun kirjoittamiseen "
            "jollekin näistä kolmesta, merkitse se rehellisesti [Vaatii tarkistuksen] "
            "äläkä keksi yksityiskohtia. Muita YVL-ohjeita (STUK:n koko 46 ohjeen "
            "sarjasta vain nämä kolme on sisällytetty tähän tietokantaan) ei saa "
            "mainita tässä kohdassa ikään kuin niistäkin olisi sisältöä käytettävissä.\n"
            "- SE-erityinen: SSM (Strålsäkerhetsmyndigheten) on Ruotsin ydin- ja "
            "säteilyturvallisuusviranomainen SMR-komponentille; tarkkaa SSMFS-numerointia "
            "ei ole vahvistettu [Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            "'thermal runaway' ja 'BMS': BESS-osahankkeen paloturvallisuus — "
            "Battery Management System (BMS), terminen eristys, H₂/CO-seuranta, "
            ...
            "- SE-erityinen: BESS-osan palo- ja räjähdysturvallisuutta säätelee Lag "
            "(2010:1011) om brandfarliga och explosiva varor (LBE), jota valvoo MSB.\n"
            "- LV-erityinen: SMR-komponentille ei ole vakiintunutta STUKia vastaavaa "
            "lupaprosessia (ks. hanketyyppi SMR:n LV-kohta samasta syystä), mutta "
            "todellinen säteilyturvallisuuslaki ja Radiācijas drošības centrs -"
            "viranomainen ovat olemassa. BESS-osan paloturvallisuuslausunnon myöntää "
            "VUGD [Ugunsdrošības, ugunsdzēsības un glābšanas darbu likums, voimassa "
            "13.11.2025 alkaen].\n\n"
            ...
            "§ 20 käyttölupa (STUK).\n"
            "- SE-erityinen: SMR-komponentin vastaava sääntely on Lag (1984:3) om "
            "kärnteknisk verksamhet, jota valvoo SSM. Ruotsissa on toisin kuin useissa "
            "muissa tässä käsitellyissä maissa aktiivisia toimivia ydinvoimaloita "
            "(Ringhals, Forsmark, Oskarshamn).\n"
            "- DE-erityinen: Atomgesetz (AtG) KIELTÄÄ uusien kaupallisten "
            "ydinvoimaloiden rakentamisen sähköntuotantoon — Saksa toteutti "
            "ydinvoimasta luopumisen 15.4.2023. Hybridihankkeen SMR-komponentille EI "
            "ole tavallista, vaikeaa lupapolkua — nimenomainen kielto koskee koko "
            "SMR-osaa samalla tavalla kuin puhdasta SMR-hanketta [ks. hanketyyppi "
            "'SMR', KERROS 2]. Hankkeen BESS-komponentti sen sijaan on Saksassa "
            "täysin lupakelpoinen tavallisen akkuvarastosääntelyn kautta [ks. "
            "hanketyyppi 'BESS'-osan DE-erityiset kohdat] — kielto koskee ainoastaan "
            "ydinreaktoriosaa, ei koko hybridihanketta.\n"
            "- PAKOLLINEN osiossa '2. Perustelut' — kirjoita tekstiin pakollisesti sanat "
            "'BAT-päätelmät': BESS-komponentin ympäristöluvassa sovellettava BAT "
            "(Best Available Techniques) solujen termiselle hallinnalle, "
            ...
            "- SE-erityinen: BESS-komponentin ympäristölupa (miljöfarlig verksamhet) "
            "käsitellään Miljöbalken kap. 9 mukaisesti.\n"
            "- LV-erityinen: BESS-komponentin piesārņojošas darbības atļauja "
            "[Likums \"Par piesārņojumu\" (2001)] — VVD myöntää.\n"
            "- DBA-analyysi (Design Basis Accident): radioaktiivisten päästöjen mallintaminen.\n\n"
            ...
            "- SE-erityinen: Ruotsin vastine periaatepäätökselle on hallituksen "
            "(regeringen) tillståndsprövning Lag (1984:3) om kärnteknisk verksamhet "
            "-lain nojalla, ei erillinen ydinturvallisuusviranomainen.\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            "'Tukes S10-ohje': BESS-komponentin palonsammutus perustuu Tukes S10-ohjeeseen; "
            ...
            "- SE-erityinen: Tukes S10-ohjetta vastaavaa ruotsalaista teknistä "
            "ohjeistusta BESS-komponentille ei ole vahvistettu [Vaatii tarkistuksen]; "
            "pelastussuunnitelma hyväksytetään paikallisella räddningstjänstillä.\n"
            "- STUK rakentamislupa [YEL § 18] ja käyttölupa [YEL § 20] SMR-komponentille.\n"
            "- Fingrid SJV-liittymäpyyntö koko yhdistetyn tehon osalta.\n"
            "- LV-erityinen: BESS-komponentin verkkoliityntä AST:n tai Sadales tīkls "
            "AS:n kautta [Elektroenerģijas tirgus likums §§73-83]; SMR-komponentille "
            "ei vakiintunutta lupaprosessia [ks. KERROS 1].\n\n"
            ...
            "- SE-erityinen: Kokonaisaikataulu [Vaatii tarkistuksen] — Ruotsissa on "
            "vakiintunut, toimiva ydinvoimaprosessi olemassa olevien laitosten osalta, "
            "mutta yhdistetyn SMR+BESS-hankkeen tarkkaa kokonaisaikataulua ei ole "
            "vahvistettu.\n"
            "- LV-erityinen: Kokonaisaikataulu ei ole arvioitavissa SMR-komponentin "
            "vakiintuneen prosessin puuttuessa [ks. hanketyyppi SMR]; BESS-komponentti "
            "seuraisi hanketyypin BESS mukaista 6–18 kk aikataulua erikseen."
        ),
    },
    "ymparistolupa": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: Vastaava toimintakuvaus ja päästöluettelo toimitetaan "
            "ympäristölupahakemuksen yhteydessä Miljöbalken kap. 9 mukaisesti.\n"
            ...
            "- SE-erityinen: IED-direktiivin BAT-päätelmät koskevat myös Ruotsia "
            "EU-jäsenmaana; kansallinen soveltaminen tehdään Miljöbalken kap. 9 nojalla.\n"
            "- LV-erityinen: IED-direktiivin (2010/75/EU) kansallinen täytäntöönpanolaki "
            "on Likums \"Par piesārņojumu\" (2001) — A-kategorian laitokset lueteltu lain "
            "omassa liitteessä 1, B/C-kategoriat MK noteikumi Nr. 1082:n liitteissä. "
            "HUOM: ei Vides aizsardzības likums (2006) — se on yleinen kehyslaki, ei "
            "operatiivinen lupalaki.\n"
            "- PAKOLLINEN osiossa '2. Perustelut' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Ympäristölupa (miljöfarlig verksamhet) käsitellään "
            "Länsstyrelsenin Miljöprövningsdelegationenissa (MPD) suurille laitoksille, "
            "tai kunnan ympäristönsuojeluviranomaisessa pienemmille, Miljöbalken kap. 9 "
            "mukaisesti; tarkka käsittelyaika [Vaatii tarkistuksen].\n"
            "- LV-erityinen: Valsts vides dienests (VVD) myöntää piesārņojošas "
            "darbības atļauja -luvan A/B/C-kategorian mukaan [Likums \"Par "
            "piesārņojumu\" (2001)]; tarkka käsittelyaika [Vaatii tarkistuksen].\n"
            "- DE-erityinen: Ympäristölupa (BImSchG) haetaan kahdella eri "
            "menettelyllä sen mukaan, mihin sarakkeeseen 4. BImSchV:n Liite 1 -listalla "
            "laitostyyppi kuuluu: Spalte 1 -laitokset käsitellään muodollisessa "
            "menettelyssä [BImSchG § 10] JULKISINE KUULEMISINEEN (hanke julkaistaan "
            "virallisessa lehdessä, sanomalehdessä ja internetissä; kuka tahansa "
            "vaikutuspiirissä oleva voi tehdä muistutuksen), kun taas Spalte 2 "
            "-laitokset saavat yksinkertaistetun menettelyn ilman julkista kuulemista "
            "[BImSchG § 19]. Kumpi menettely soveltuu, riippuu siis suoraan "
            "laitostyypin ja -kapasiteetin luokituksesta Liite 1:ssä "
            "[Vaatii tarkistuksen laitostyypin tarkan sarakeluokituksen osalta].\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Länsstyrelsenin lausuntojen ja kuulemisen tarkka kesto "
            "[Vaatii tarkistuksen].\n"
    ...
    "datakeskus": {
        ...
        "context_extra": (
            ...
            "- SE-erityinen: EU 2023/1791 on asetus, joten se koskee sellaisenaan myös "
            "Ruotsia EU-jäsenmaana ilman erillistä kansallista täytäntöönpanoa.\n"
            "- DE-erityinen: Saksalla on EU-asetuksen lisäksi oma, tiukempi kansallinen laki, "
            "Energieeffizienzgesetz (EnEfG, voimassa 18.11.2023 alkaen). Konkreettiset "
            "sitovat vaatimukset datakeskuksille: PUE ≤ 1,5 viimeistään 1.7.2027 (ennen "
            "1.7.2026 käyttöönotetut datakeskukset) tai PUE ≤ 1,2 viimeistään 2 vuoden "
            "kuluttua käyttöönotosta (1.7.2026 jälkeen käyttöönotettavat); hukkalämmön "
            "hyötykäyttöosuus vähintään 10 % (2026), 15 % (2027), 20 % (2028); "
            "uusiutuvan sähkön kattavuus 50 % (1.1.2024 alkaen) nousten 100 %:iin "
            "(1.1.2027 alkaen); raportointivelvollisuus BAFA:lle (Bundesamt für "
            "Wirtschaft und Ausfuhrkontrolle). Suomessa vastaavaa kansallista sitovaa "
            "PUE-/hukkalämpölakia ei ole [Vaatii tarkistuksen hankkeen käyttöönottoajankohdan "
            "mukaisen tarkan kynnysarvon osalta].\n"
            "- PAKOLLINEN osiossa '1. Hankkeen kuvaus' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Vastaava harkinta (miljöbedömning) tehdään Miljöbalken kap. 6 "
            "mukaisesti; kynnysarvo ja käsittelevä viranomainen (Länsstyrelsen) "
            "[Vaatii tarkistuksen].\n"
            "- PAKOLLINEN osiossa '2. Perustelut' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Ruotsissa kaavamuutos ja rakennuslupa käsitellään yhden lain, "
            "Plan- och bygglagen (PBL, 2010:900), nojalla — ei kahta erillistä prosessia "
            "kuten Suomessa; kunta (kommun) käsittelee sekä detaljplaneändringen että bygglovet.\n"
            "- PAKOLLINEN osiossa '3. Luvat' — kirjoita tekstiin pakollisesti sanat "
            ...
            "- SE-erityinen: Kantaverkkoliittymän vastapuoli on Svenska kraftnät "
            "(Ruotsin Fingrid-vastine); jakeluverkkoliittymä paikallisen verkkoyhtiön kanssa.\n"
            "- Ympäristölupa [YSL 527/2014]: tarvitaan jäähdytysveden käytölle tai "
            "merkittävälle melulle.\n"
            "- SE-erityinen: Vastaava ympäristölupa (miljöfarlig verksamhet) käsitellään "
            "Miljöbalken kap. 9 mukaisesti Länsstyrelsenin Miljöprövningsdelegationenissa "
            "(MPD) tai kunnassa laitoksen koosta riippuen.\n"
            "- Rakentamislupa: haettava Lupapiste.fi-palvelusta [Rakentamislaki 751/2023, 50 §].\n"
            "- SE-erityinen: Vastaava rakennuslupa (bygglov) haetaan kunnalta PBL 2010:900 "
            "mukaisesti.\n"
            "- LV-erityinen: Kaavoitusvaatimus on detālplānojums, tarvittaessa "
            "[Teritorijas attīstības plānošanas likums (2011)]; rakennuslupa "
            "(būvatļauja) pašvaldības būvvaldelta [Būvniecības likums (2013)]; "
            "verkkoliityntä AST:n tai Sadales tīkls AS:n kautta koon mukaan "
            "[Elektroenerģijas tirgus likums §§73-83]; ympäristölupa tarvittaessa "
            "VVD:ltä [Likums \"Par piesārņojumu\" (2001)].\n\n"
            ...
            "- SE-erityinen: Vaiheiden 2, 5 ja 6 tarkat käsittelyajat Ruotsissa "
            "[Vaatii tarkistuksen]."
        ),
        "luvat": [
            ("Rakentamislupa",                     "Kunta / rakennusvalvonta",    "Rakentamislaki 751/2023"),
    ...
    "teollisuus": {
        ...
        "context_extra": (
            ...
            "- LV-erityinen: IED-direktiivin kansallinen täytäntöönpanolaki on Likums "
            "\"Par piesārņojumu\" (2001) — A-kategorian laitokset lain omassa liitteessä "
            "1, B/C-kategoriat MK noteikumi Nr. 1082:ssa; VVD myöntää luvan.\n"
            "- DE-erityinen: Ilmapäästöjen tekninen raja-arvosto perustuu TA Luftiin "
            "(Technische Anleitung zur Reinhaltung der Luft) — BImSchG:n yleinen "
            "hallinnollinen määräys, joka koskee yli 50 000:ta lupavelvollista "
            "laitosta Saksassa ja jonka nykyinen versio on voimassa 1.12.2021 alkaen "
            "(EU:n teollisuuspäästödirektiivin IED vaatimusten mukaisesti "
            "päivitetty). TA Luft asettaa sitovat päästö-/immissioraja-arvot ja "
            "niiden mittausvaatimukset — vastaava rooli kuin IED:n BAT-päätelmillä, "
            "mutta kansallisena, yksityiskohtaisena hallinnollisena määräyksenä eikä "
            "suoraan sovellettavana EU-säädöksenä [Vaatii tarkistuksen laitostyypin "
            "tarkkojen raja-arvojen osalta].\n"
            ...
            "- LV-erityinen: Kaavoitusvaatimus on detālplānojums [Teritorijas "
            "attīstības plānošanas likums (2011)]; rakennuslupa (būvatļauja) "
            "pašvaldības būvvaldelta [Būvniecības likums (2013)].\n\n"
...
_COUNTRY_CONFIG: dict[str, dict] = {
    ...
    "PL": {
        ...
        "key_laws": ["Prawo atomowe (Dz.U. 2001 nr 3 poz. 18)", "Prawo budowlane (Dz.U. 1994 nr 89 poz. 414)",
                     "Ustawa o planowaniu i zagospodarowaniu przestrzennym (Dz.U. 2003 nr 80 poz. 717)",
                     "Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)", "Prawo ochrony środowiska (Dz.U. 2001 nr 62 poz. 627)",
                     "Prawo wodne (Dz.U. 2017 poz. 1566)", "Ustawa o ochronie przeciwpożarowej (Dz.U. 1991 nr 81 poz. 351)",
                     "Ustawa o OZE (20.02.2015)"],
        # 2026-08-23 (item 5(B), PL research — see PL_LAW_CITATION_RESEARCH.md):
        # same fix as SE's prompt_prefix (PR #106) -- this already listed real
        # Polish laws but didn't map which Finnish statute each one replaces,
        # so the model kept citing "MRL 132/1999"/"YSL 527/2014" etc. directly
        # despite having the correct raw material in context. Now explicit.
        "prompt_prefix": (
            "IMPORTANT — COUNTRY: This project is located in POLAND. Apply Polish regulatory framework:\n"
            "Key authorities: PAA (State Nuclear Agency, for nuclear projects — decisions issued by the "
            "President of PAA/Prezes PAA), Minister Klimatu i Środowiska (Ministry of Climate and "
            "Environment — issues the nuclear 'decyzja zasadnicza'/decision-in-principle; equivalent role "
            "to Finland's Council of State decision-in-principle, NOT 'Rada Stanu', which is not a real "
            "Polish governing body for this), "
            ...
            "RDOŚ (Regional Environmental Directorate) / GDOŚ (national), "
            "Starosta lub Wojewoda (poviat/district authority for building permits = Pozwolenie na budowę; "
            "for major infrastructure like nuclear facilities, jurisdiction escalates to the Wojewoda level), "
            "Wody Polskie (Państwowe Gospodarstwo Wodne — water-law permits), "
            "IMGW-PIB / Centrum Technicznej Kontroli Zapór (dam safety technical oversight, under Prawo "
            "wodne — Poland has NO separate dam-safety act), "
            "Państwowa Straż Pożarna PSP (fire safety), "
            ...
            "Key laws — use EXACTLY these, mapped to the Finnish statute each one replaces:\n"
            "- Ustawa o planowaniu i zagospodarowaniu przestrzennym (Dz.U. 2003 nr 80 poz. 717) = "
            "spatial/zoning planning — replaces Finnish MRL 132/1999. Poland, unlike Sweden, keeps this "
            "SEPARATE from building permits (two different acts, not one combined law).\n"
            "- Prawo budowlane (Dz.U. 1994 nr 89 poz. 414) = building permits (pozwolenie na budowę) — "
            "replaces Finnish Rakentamislaki 751/2023.\n"
            "- Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227) = environmental impact assessment — "
            "replaces Finnish YVA-laki 252/2017.\n"
            "- Prawo ochrony środowiska POŚ (Dz.U. 2001 nr 62 poz. 627) = environmental permit "
            "(pozwolenie zintegrowane for larger installations) — replaces Finnish YSL 527/2014. Do NOT "
            "use 'Ustawa o ochronie środowiska' -- that is not the real title.\n"
            "- Prawo wodne (Dz.U. 2017 poz. 1566) = water-law permits AND dam safety (Poland has no "
            "separate dam-safety act) — replaces Finnish Vesilaki 587/2011 and Patoturvallisuuslaki 494/2009.\n"
            "- Ustawa o ochronie przeciwpożarowej (Dz.U. 1991 nr 81 poz. 351) = fire safety — "
            "replaces Finnish Pelastuslaki 379/2011.\n"
            "- Prawo atomowe (Dz.U. 2001 nr 3 poz. 18) = nuclear technical activities, reviewed by PAA — "
            "replaces Finnish Ydinenergialaki 990/1987. Do NOT use 'Ustawa o energii jądrowej' -- that is "
            "not the real title.\n"
            "- Ustawa o OZE (20.02.2015) = renewable energy support scheme. Rozporządzenie w sprawie "
            "przeszkód lotniczych, Dz.U. 2021 poz. 264 = aviation obstacle marking.\n"
            "CRITICAL: Never cite a Finnish statute number (MRL, YSL, YVA-laki, Rakentamislaki, Vesilaki, "
            "Pelastuslaki, Ydinenergialaki, Patoturvallisuuslaki) directly in the output — always use the "
            "correct Polish law from the mapping above, and never invent a plausible-sounding Polish law "
            "name that is not in the list above.\n"
            "Replace ALL Finnish entity references (ELY-keskus, Tukes, Traficom, Fingrid, STUK, Luova) "
            "with the Polish equivalents listed above (RDOŚ, UDT, ULC, PSE S.A., PAA — not URE, which is "
            "the regulator rather than the grid operator). "
    ...
    "EE": {
        ...
        "authorities": [
            ...
            "from 01.01.2027 also hosts Estonia's new independent nuclear regulator "
            "under TEOS, see key_laws below)",
        ],
        # 2026-08-26: key_laws corrected/expanded after real research (see
        # EE_LAW_CITATION_RESEARCH.md) -- sourced from cross-checked secondary
        # sources, not fetched primary text, since riigiteataja.ee is
        # JS-rendered (same blocker as LT/DA); lower-confidence sourcing tier,
        # see the note above _LAW_CITATION_REPLACEMENT.
        "key_laws": [
            "Elektrituruseadus (ETS, RT I 2012) — electricity market, production license",
            "Keskkonnaseadustiku üldosa seadus (KeÜS, RT I 28.02.2011,1) — general "
            "environmental framework, defines the general permit concept only",
            # Correction: KeÜS alone was previously (wrongly) cited as the
            # operative permit law for industrial facilities. Tööstusheite
            # seadus is the actual operating-permit law (kompleksluba /
            # keskkonnaluba above/below the IED capacity threshold).
            "Tööstusheite seadus (RT I 16.05.2013, in force 01.06.2013) — the "
            "operative industrial/IPPC permit-granting law (implements EU IED "
            "2010/75/EU); this is what actually grants the permit, not KeÜS alone",
            ...
            "Veeseadus (RT I 2019, in force 01.10.2019) — water-law permits; "
            "genuinely new to this file, no vesivoima row existed before",
            # Correction: was misspelled "Tuleohutusseadus" (one word) in the
            # per-hanketyyppi tables; real name is two words, genitive case.
            "Tuleohutuse seadus (RT I 2010, in force 01.09.2010) — fire safety "
            "requirements/inspections (NOT Päästeseadus, a same-day companion "
            "act governing the Rescue Board's rescue-operation powers instead)",
            # Major correction: was previously described as "no nuclear power
            # law yet (draft under development)" -- that's stale. The law has
            # been ADOPTED, just isn't in force yet. State both facts.
            "Tuumaenergia ja -ohutuse seadus (TEOS) — adopted by Riigikogu "
            "17.06.2026 (third reading, 63-10-1), ENTERS INTO FORCE 01.01.2027 "
            "(not yet in force as of writing); creates a tiered nuclear "
            "licensing system and a new nuclear regulator inside TTJA",
        ],
        "prompt_prefix": (
            ...
            "defines the general permit concept only, NOT the operative industrial "
            "permit law, see Tööstusheite seadus below), "
            "Tööstusheite seadus (the actual industrial/IPPC permit-granting law — "
            "kompleksluba/keskkonnaluba, implements EU IED 2010/75/EU), "
            "Veeseadus (water-law permits), "
            "Tuleohutuse seadus (fire safety — NOT Päästeseadus, a different, "
            "same-day companion law about the Rescue Board's operational powers), "
            "Energiamajanduse korralduse seadus ESOS (energy sector organisation + NECP targets).\n"
            "Key laws — explicit FI→EE mapping (replace the Finnish citation with the "
            "matching Estonian one below, never leave the Finnish law name/number in "
            "the output): "
            "MRL 132/1999 -> Planeerimisseadus (PlanS, 2015); "
            "Rakentamislaki 751/2023 -> Ehitusseadustik (EhS, 2015); "
            "YVA-laki 252/2017 -> KMH-KSH seadus (RT I, 04.01.2013, 10); "
            "YSL 527/2014 -> Tööstusheite seadus (2013; NOT Keskkonnaseadustiku üldosa "
            "seadus/KeÜS alone, which is the general framework act, not the "
            "permit-granting one); "
            "Vesilaki 587/2011 -> Veeseadus (2019); "
            "Pelastuslaki 379/2011 -> Tuleohutuse seadus (2010; NOT Päästeseadus); "
            "Patoturvallisuuslaki 494/2009 -> [Requires verification against Estonian "
            "regulations] — not yet researched, do not invent a law name; "
            "Sähkömarkkinalaki 588/2013 -> Elektrituruseadus (ETS, RT I 2012).\n"
            ...
            "SMR/nuclear: Estonia's Tuumaenergia ja -ohutuse seadus (TEOS) was ADOPTED "
            "by the Riigikogu on 17.06.2026 (third reading, 63 in favour, 10 against, "
            "1 abstention) — this is NOT a live draft, it is a completed legislative "
            "act. It is NOT YET IN FORCE as of today; entry into force is a firm, "
            "already-scheduled date of 01.01.2027. TEOS creates a tiered licensing "
            "system (preliminary assessment -> construction permit -> testing permit "
            "-> operating permit -> decommissioning permit) and a new independent "
            "nuclear regulator inside TTJA, operating from 01.01.2027. Any decision "
            "to actually build a plant requires separate Riigikogu approval. State "
            "the exact status explicitly — 'adopted 17.06.2026, in force 01.01.2027' "
            "— never collapse this to either 'no nuclear law exists' or 'a live "
            "nuclear licensing system operates today'.\n"
            "Replace ALL Finnish law/entity references (MRL, YSL, YVA-laki, Vesilaki, Pelastuslaki, "
            "Ydinenergialaki, ELY-keskus, Tukes, Traficom, Fingrid, STUK) "
    ...
    "LT": {
        ...
        "key_laws": [
            ...
            "Vandens įstatymas — water-law permits",
            "Priešgaisrinės saugos įstatymas — fire safety (PAGD ekspertizė)",
            "Branduolinės energijos įstatymas (1996, as amended) — nuclear, regulated by VATESI; see notes below",
        ],
        "prompt_prefix": (
            ...
            "Finnish-to-Lithuanian statute mapping — use EXACTLY these:\n"
            "- MRL 132/1999 -> Teritorijų planavimo įstatymas (zoning/spatial planning).\n"
            "- Rakentamislaki 751/2023 -> Statybos įstatymas (building permits). Lithuania, like Poland, "
            "keeps these as TWO SEPARATE acts — do not merge them into one combined law.\n"
            "- YVA-laki 252/2017 -> PAV įstatymas (EIA).\n"
            "- YSL 527/2014 -> Aplinkos apsaugos įstatymas (environmental permit).\n"
            "- Vesilaki 587/2011 -> Vandens įstatymas (water-law permit).\n"
            "- Pelastuslaki 379/2011 -> Priešgaisrinės saugos įstatymas (fire safety).\n"
            "- Patoturvallisuuslaki 494/2009 -> no separate Lithuanian act exists; hydrotechnical "
            "structures (hidrotechniniai statiniai) are regulated under Statybos įstatymas plus "
            "technical construction regulations (STR 2.02.06, STR 2.05.14) — cite those, do not invent "
            "a standalone dam-safety law.\n"
            "- Ydinenergialaki 990/1987 -> Branduolinės energijos įstatymas (1996, as amended) DOES "
            "exist and VATESI is a real, active regulator under it — but VATESI's current remit is "
            "Ignalina NPP decommissioning and radioactive waste/material control, NOT new-build "
            "reactor licensing. State BOTH facts together for any SMR/nuclear content: the real law "
            "and regulator exist, AND building a genuinely new nuclear facility would need new primary "
            "legislation before any permit path exists — never state only one half of this.\n"
            "Never invent a plausible-sounding Lithuanian law name that is not in the mapping above.\n"
...
_COUNTRY_LUVAT: dict[str, dict[str, list[tuple[str, str, str]]]] = {
    ...
    "EE": {
        ...
        "BESS": [
            ("Rakentamislupa (ehitusluba)",         "Kohaliku omavalitsuse (Municipality)",       "Ehitusseadustik (EhS, RT I 2015) §19"),
            ("Paloturvallisuuslupa (Päästeamet)",  "Päästeamet (Estonian Rescue Board)",          "Tuleohutuse seadus (Fire Safety Act, RT I 2010)"),
        ...
        # ── SMR — regWarning: TEOS adopted 17.06.2026, in force 01.01.2027 ─────
        # 2026-08-26 correction: previous rows said the nuclear law "puuttuu —
        # draft laki" (missing, draft) -- stale. Tuumaenergia ja -ohutuse
        # seadus (TEOS) passed its third Riigikogu reading 17.06.2026 (63-10-1)
        # and IS adopted, but is NOT YET IN FORCE (entry into force 01.01.2027).
        # See EE_LAW_CITATION_RESEARCH.md and the Ydinenergialaki comment in
        # _LAW_CITATION_REPLACEMENT above for the full status. Sourced from
        # cross-checked secondary sources (riigiteataja.ee is JS-rendered) —
        # lower-confidence sourcing tier, see the note above
        # _LAW_CITATION_REPLACEMENT.
        "SMR": [
            ("⚠️ TEOS vastu võetud 17.06.2026, EI OLE VEEL JÕUS", "Riigikogu (Parliament) — jõustub 1.1.2027", "Tuumaenergia ja -ohutuse seadus (TEOS) — tuumaregulaator TTJA koosseisus alates 1.1.2027"),
            ("Periaatepäätös (Riigikogu erikorraldus, tarvittaessa)", "Riigikogu (Parliament of Estonia)", "TEOS § — reaktori ehitamise otsus vajab eraldi Riigikogu heakskiitu"),
            ...
            ("Rakentamislupa (ehitusluba, tarvitt.)", "Kohaliku omavalitsuse / TTJA tuumaregulaator", "Ehitusseadustik (EhS) + TEOS astmeline loamenetlus (alates 1.1.2027)"),
            ("Elektrienergia tootmise luba",        "Konkurentsiamet (Competition Authority)",    "Elektrituruseadus (ETS, RT I 2012) §14"),
        ...
        "teollisuus": [
            ("Detailplaneering (tarvitt.)",         "Kohaliku omavalitsuse volikogu",             "Planeerimisseadus (PlanS, RT I 2015)"),
            # Correction 2026-08-26: KeÜS alone was previously cited here, but
            # it only defines the general permit concept -- the operative
            # industrial/IPPC permit-granting law is Tööstusheite seadus
            # (kompleksluba/keskkonnaluba above/below the IED threshold).
            ("Keskkonnaluba (environmental permit)", "Keskkonnaamet (Environmental Board)",        "Tööstusheite seadus (2013) / Keskkonnaseadustiku üldosa seadus (KeÜS, RT I 2011)"),
        ...
        "maatalous": [
            ...
            ("Keskkonnaluba (tarvitt., suuret tilat)", "Keskkonnaamet",                           "Tööstusheite seadus (2013) — IPPC-kynnys / kompleksluba"),
        ...
        "hybridi": [
            ...
            ("YVA-menettely (KMH)",                "Keskkonnaamet (Environmental Board)",         "KMH-KSH seadus (RT I, 04.01.2013, 10)"),
            ("Rakentamislupa (ehitusluba)",         "Kohaliku omavalitsuse (Municipality)",       "Ehitusseadustik (EhS, RT I 2015) §19"),
            ("Paloturvallisuuslupa (BESS-osuus)",  "Päästeamet (Estonian Rescue Board)",          "Tuleohutuse seadus (Fire Safety Act, RT I 2010)"),
            ("Elektrienergia tootmise luba",        "Konkurentsiamet (Competition Authority)",    "Elektrituruseadus (ETS, RT I 2012) §14"),
...
_COUNTRY_LIITTEET: dict[str, dict[str, list[str]]] = {
    ...
    "PL": {
        "SMR": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Raport bezpieczeństwa (PAA — Państwowa Agencja Atomistyki) — Prawo atomowe (Dz.U. 2001 nr 3 poz. 18)",
            "Raport o oddziaływaniu na środowisko (OOŚ) — Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)",
        ...
        "smr_bess": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Raport bezpieczeństwa (PAA — Państwowa Agencja Atomistyki) — Prawo atomowe (Dz.U. 2001 nr 3 poz. 18)",
            "Raport OOŚ — Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)",
            "Raport bezpieczeństwa pożarowego BESS (NFPA 855 / EN 50604-1) — zgodnie z Ustawą o ochronie "
            "przeciwpożarowej (Dz.U. 1991 nr 81 poz. 351)",
            "Plan przyłączenia do sieci (PSE S.A.)",
            "Miejscowy plan zagospodarowania przestrzennego (MPZP)",
            "Odpis z KRS / CEIDG",
        ],
        ...
        # 2026-08-23 (item 4, PL research — see PL_LAW_CITATION_RESEARCH.md):
        # energy-project hanketyyppis added first per pilot-activity priority
        # (PL/AP1000 pipeline); construction types deliberately not yet
        # added, lower priority. All citations verified against Sejm/ISAP,
        # GDOŚ, PAA — see the research doc for sourcing.
        "BESS": [
            "Mapa lokalizacyjna (skala 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "Zgłoszenie/pozwolenie na budowę — Prawo budowlane (Dz.U. 1994 nr 89 poz. 414)",
            "Raport bezpieczeństwa pożarowego BESS — zgodnie z Ustawą o ochronie przeciwpożarowej "
            "(Dz.U. 1991 nr 81 poz. 351)",
            "Pozwolenie zintegrowane lub zgłoszenie instalacji (w zależności od skali) — Prawo ochrony "
            "środowiska POŚ (Dz.U. 2001 nr 62 poz. 627)",
            "Miejscowy plan zagospodarowania przestrzennego (MPZP) — Ustawa o planowaniu i "
            "zagospodarowaniu przestrzennym (Dz.U. 2003 nr 80 poz. 717)",
            "Plan przyłączenia do sieci (PSE S.A. / lokalny operator sieci dystrybucyjnej)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        "tuulivoima_maa": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Raport o oddziaływaniu na środowisko (OOŚ) — Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)",
            "Decyzja o środowiskowych uwarunkowaniach (DŚU) — wydawana przez RDOŚ/GDOŚ",
            "Miejscowy plan zagospodarowania przestrzennego (MPZP) — Ustawa o planowaniu i "
            "zagospodarowaniu przestrzennym (Dz.U. 2003 nr 80 poz. 717)",
            "Pozwolenie na budowę — Prawo budowlane (Dz.U. 1994 nr 89 poz. 414)",
            "Plan przyłączenia do sieci (PSE S.A.)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        "tuulivoima_meri": [
            "Mapa lokalizacyjna (mapa morska)",
            "Maankäyttöselvitys PDF (NCE)",
            "Raport o oddziaływaniu na środowisko (OOŚ) — Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)",
            "Pozwolenie lokalizacyjne dla morskich farm wiatrowych (Urząd Morski / Prawo wodne, "
            "Dz.U. 2017 poz. 1566)",
            "Koncesja na wytwarzanie energii z morskich farm wiatrowych (URE)",
            "Plan przyłączenia do sieci (PSE S.A.)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        "aurinkovoima": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Zgłoszenie/pozwolenie na budowę — Prawo budowlane (Dz.U. 1994 nr 89 poz. 414)",
            "Raport o oddziaływaniu na środowisko (w razie potrzeby, większe instalacje) — Ustawa OOŚ "
            "(Dz.U. 2008 nr 199 poz. 1227)",
            "Miejscowy plan zagospodarowania przestrzennego (MPZP) lub decyzja o warunkach zabudowy",
            "Plan przyłączenia do sieci (lokalny operator sieci dystrybucyjnej)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        "vesivoima": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Raport o oddziaływaniu na środowisko (OOŚ) — Ustawa OOŚ (Dz.U. 2008 nr 199 poz. 1227)",
            "Pozwolenie wodnoprawne — Prawo wodne (Dz.U. 2017 poz. 1566, wydawane przez Wody Polskie)",
            "Dokumentacja bezpieczeństwa zapory — Prawo wodne (Dz.U. 2017 poz. 1566; nadzór "
            "IMGW-PIB / Centrum Technicznej Kontroli Zapór — Polska nie posiada odrębnej ustawy o "
            "bezpieczeństwie zapór)",
            "Plan przyłączenia do sieci (PSE S.A.)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        # hybridi (BESS + wind/solar): real de-duplicated combined list, same
        # approach as SE's -- not a mechanical union of the component lists.
        "hybridi": [
            "Mapa lokalizacyjna (skala 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "Raport o oddziaływaniu na środowisko (w razie potrzeby, zależnie od skali) — Ustawa OOŚ "
            "(Dz.U. 2008 nr 199 poz. 1227)",
            "Pozwolenie zintegrowane lub zgłoszenie instalacji (część BESS i wytwórcza) — Prawo "
            "ochrony środowiska POŚ (Dz.U. 2001 nr 62 poz. 627)",
            "Raport bezpieczeństwa pożarowego BESS — zgodnie z Ustawą o ochronie przeciwpożarowej "
            "(Dz.U. 1991 nr 81 poz. 351)",
            "Miejscowy plan zagospodarowania przestrzennego (MPZP) — Ustawa o planowaniu i "
            "zagospodarowaniu przestrzennym (Dz.U. 2003 nr 80 poz. 717)",
            "Pozwolenie na budowę — Prawo budowlane (Dz.U. 1994 nr 89 poz. 414)",
            "Zintegrowany plan przyłączenia do sieci (PSE S.A. / lokalny operator sieci)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
        "ymparistolupa": [
            "Pozwolenie zintegrowane lub pozwolenie sektorowe — Prawo ochrony środowiska POŚ "
            "(Dz.U. 2001 nr 62 poz. 627)",
            "Raport o oddziaływaniu na środowisko (w razie potrzeby) — Ustawa OOŚ (Dz.U. 2008 nr 199 "
            "poz. 1227)",
            "Odpis z KRS / CEIDG",
            "Pełnomocnictwo (jeżeli reprezentant działa w imieniu wnioskodawcy)",
        ],
    ...
    "EE": {
        "SMR": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "⚠️ Tuumaenergia ja -ohutuse seadus (TEOS) vastu võetud 17.06.2026, EI OLE VEEL JÕUS — jõustub 1.1.2027; enne seda ei ole reaktori loamenetlus TTJA kaudu veel avatud",
            ...
            "⚠️ TEOS jõustub alles 1.1.2027 — kuni selleni ei ole täielikku tuumaseadusandlikku raamistikku rakendatav; ehitusotsus vajab lisaks eraldi Riigikogu heakskiitu",
        ],
        "smr_ee": [
            "Asukohakaart / Sijaintikartta (mõõtkava 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
        ...
        # 2026-08-26 build-out: BESS, aurinkovoima, vesivoima, hybridi,
        # ymparistolupa were missing entirely (EE previously had only 5 of
        # the standard 10 _COUNTRY_LIITTEET keys). Same shape and content
        # depth as LV's own build. Sourced from cross-checked secondary
        # sources (riigiteataja.ee is JS-rendered) -- lower-confidence
        # sourcing tier, see the note above _LAW_CITATION_REPLACEMENT.
        "BESS": [
            "Asukohakaart / Sijaintikartta (mõõtkava 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "Ehitusluba — Ehitusseadustik (EhS, RT I 2015) §19",
            "Keskkonnaluba või teavitus (suurusest olenevalt) — Tööstusheite seadus (2013)",
            "Tuleohutusdokumentatsioon (NFPA 855 / EN 50604-1) — Tuleohutuse seadus (RT I 2010)",
            "Detailplaneering",
            "Liitumisleping Elering AS-iga / Elektrilevi OÜ-ga",
            "Äriregistri väljavõte (Company Registry extract)",
            "Volikiri (if representative acts on behalf of applicant)",
        ],
        "aurinkovoima": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Ehitusluba — Ehitusseadustik (EhS, RT I 2015) §19",
            "KMH aruanne (tarvitt., suurte objektide puhul) — KMH-KSH seadus §7",
            "Detailplaneering (tarvitt.)",
            "Liitumisleping Elektrilevi OÜ-ga / Elering AS-iga",
            "Äriregistri väljavõte (Company Registry extract)",
            "Volikiri (if representative acts on behalf of applicant)",
        ],
        "vesivoima": [
            "Asukohakaart / Sijaintikartta (mõõtkava 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "KMH aruanne — KMH-KSH seadus",
            "Veekasutusluba — Veeseadus (RT I 2019)",
            "Liitumisleping Elering AS-iga",
            "Äriregistri väljavõte (Company Registry extract)",
            "Volikiri (if representative acts on behalf of applicant)",
        ],
        "hybridi": [
            "Asukohakaart / Sijaintikartta (mõõtkava 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "KMH aruanne (suurusest ja tuulekomponendist olenevalt) — KMH-KSH seadus",
            "Keskkonnaluba või teavitus (BESS ja tootmisosa) — Tööstusheite seadus (2013)",
            "Tuleohutusdokumentatsioon (BESS-osa) — Tuleohutuse seadus (RT I 2010)",
            "Detailplaneering",
            "Ehitusluba — Ehitusseadustik (EhS, RT I 2015) §19",
            "Integreeritud liitumisleping Elering AS-iga / Elektrilevi OÜ-ga",
            "Äriregistri väljavõte (Company Registry extract)",
            "Volikiri (if representative acts on behalf of applicant)",
        ],
        "ymparistolupa": [
            "Keskkonnaluba või valdkondlik luba — Tööstusheite seadus (2013) / "
            "Keskkonnaseadustiku üldosa seadus (KeÜS)",
            "KMH aruanne (tarvitt.) — KMH-KSH seadus",
            "Äriregistri väljavõte (Company Registry extract)",
            "Volikiri (if representative acts on behalf of applicant)",
        ],
    },
    ...
    # LT had zero _COUNTRY_LIITTEET coverage for ANY hanketyyppi. Energy
    # types added first per pilot-activity priority; SMR/smr_bess/hybridi
    # deliberately NOT added here -- SMR/smr_bess would require enumerating
    # specific attachment documents for a nuclear permit PROCESS that does
    # not demonstrably exist yet in Lithuania (see the kaava_SMR card and
    # _LAW_CITATION_REPLACEMENT["Ydinenergialaki"]["LT"] for the honest
    # "real law + real regulator + no new-build path" treatment) -- listing
    # specific required attachments for a process that isn't established
    # would itself be a form of fabrication. hybridi grouped alongside per
    # explicit instruction, though it has no actual nuclear content of its
    # own -- flagged as a lower-confidence exclusion, easy to add later if
    # wanted. Construction types (asuinrakennus/liikerakennus/maatalous/
    # teollisuus) also deferred, same sequencing as SE/PL. All citations
    # grounded in the already-well-researched _COUNTRY_CONFIG["LT"]
    # prompt_prefix (VERT/LITGRID/ESO/AAA/PAGD thresholds and authorities).
    "LT": {
        "BESS": [
            "Vietos schema (mastelis 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "Statybos leidimas — Statybos įstatymas (per INFOSTATYBA)",
            "Priešgaisrinės saugos ekspertizė — Priešgaisrinės saugos įstatymas (PAGD, privaloma prieš "
            "statybos leidimą)",
            "PAV atranka (rekomenduojama >10 MWh) — PAV įstatymas (AAA)",
            "Prisijungimo prie tinklo sutartis (LITGRID >5 MW arba ESO <5 MW)",
        ...
        "tuulivoima_maa": [
            "Vietos schema (mastelis 1:20 000)",
            "Maankäyttöselvitys PDF (NCE)",
            "PAV ataskaita — PAV įstatymas (privaloma >5 vėjo jėgainių arba >30 MW, AAA)",
            "Bendrasis planas (turi numatyti vėjo energetikos zonas) — Teritorijų planavimo įstatymas",
            "Detalusis planas (paprastai reikalingas >1 vėjo jėgainei)",
            "Statybos leidimas — Statybos įstatymas (per INFOSTATYBA)",
            "Karinio radaro derinimas (Lietuvos kariuomenė — privaloma visiems vėjo parkams)",
            "Aviacijos kliūčių derinimas (ANCO/CAA — vėjo jėgainėms >45 m)",
            "Prisijungimo prie tinklo sutartis (LITGRID >5 MW arba ESO <5 MW)",
            "Juridinio asmens registravimo pažymėjimas",
            "Įgaliojimas (jei atstovas veikia pareiškėjo vardu)",
        ],
        "tuulivoima_meri": [
            "Vietos schema (jūrlapis)",
            "Maankäyttöselvitys PDF (NCE)",
            "PAV ataskaita — PAV įstatymas (AAA)",
            "Vandens įstatymu grindžiamas leidimas (jūrinei teritorijai)",
            "Karinio radaro derinimas (Lietuvos kariuomenė)",
            "Prisijungimo prie tinklo sutartis (LITGRID)",
            "Juridinio asmens registravimo pažymėjimas",
            "Įgaliojimas (jei atstovas veikia pareiškėjo vardu)",
        ],
        "aurinkovoima": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "Statybos leidimas — Statybos įstatymas (per INFOSTATYBA)",
            "PAV atranka (privaloma >50 MW) — PAV įstatymas (AAA)",
            "Teritorijų planavimo dokumentas (bendrasis arba detalusis planas)",
            "Prisijungimo prie tinklo sutartis (ESO arba LITGRID, priklausomai nuo galios)",
            "Juridinio asmens registravimo pažymėjimas",
            "Įgaliojimas (jei atstovas veikia pareiškėjo vardu)",
        ],
        "vesivoima": [
            ...
            "Maankäyttöselvitys PDF (NCE)",
            "PAV ataskaita — PAV įstatymas (AAA)",
            "Vandens naudojimo leidimas — Vandens įstatymas",
            "Hidrotechninio statinio saugos dokumentacija — Statybos įstatymas ir techniniai reglamentai "
            "(STR 2.02.06, STR 2.05.14; Lietuva neturi atskiro užtvankų saugos įstatymo)",
            "Prisijungimo prie tinklo sutartis (LITGRID)",
            "Juridinio asmens registravimo pažymėjimas",
            "Įgaliojimas (jei atstovas veikia pareiškėjo vardu)",
        ],
...
# 2026-08-23 (item 4, PL research): same offshore_wind/tuulivoima_meri
# aliasing as SE above.
_COUNTRY_LIITTEET["PL"]["offshore_wind"] = _COUNTRY_LIITTEET["PL"]["tuulivoima_meri"]
...
_LAW_TRANS: dict[str, dict[str, str]] = {
    ...
    # 2026-08-26: DE entry added here to mirror _LAW_CITATION_REPLACEMENT's
    # own Patoturvallisuuslaki["DE"] entry, added during the DE build but
    # missed here at the time -- caught by dogfooding
    # scripts/validate_country_coverage.py against real data for the first
    # time, not found by manual review.
    "Patoturvallisuuslaki 494/2009":                        {"EN": "Dam Safety Act (494/2009)",                                            "SE": "Förordning (2014:214) om dammsäkerhet",                         "DA": "[Vaatii tarkistuksen — Tanskan patoturvallisuuslainsäädäntöä ei ole vielä varmistettu]",                      "NO": "Damsikkerhetsforskriften (2009, nr. 1600) — NVE tilsyn",                    "PL": "Prawo wodne (Dz.U. 2017 poz. 1566) — brak odrębnej ustawy o bezpieczeństwie zapór w Polsce", "LV": "Likums \"Par hidroelektrostaciju hidrotehnisko būvju drošumu\" (2000/2001) — A klases būvēm uzrauga Būvniecības valsts kontroles birojs (BVKB)", "DE": "Wasserhaushaltsgesetz (WHG) i.V.m. DIN 19700 (Stauanlagen, Teil 11 Talsperren)"},
}
...
_LIITE_TRANS: dict[str, dict[str, str]] = {
    ...
    "Padon turvallisuussuunnitelma (PATL 494/2009)":         {"EN": "Dam safety plan (Dam Safety Act 494/2009)",                "SE": "Damsäkerhetsplan (Förordning (2014:214) om dammsäkerhet)",         "DA": "Dæmningssikkerhedsplan (Dæmningssikkerhedsloven 494/2009)", "NO": "Damsikkerhetsplan (Damsikkerhetsloven 494/2009)",    "PL": "Plan bezpieczeństwa zapory (Prawo wodne, Dz.U. 2017 poz. 1566 — brak odrębnej ustawy o bezpieczeństwie zapór w Polsce)"},
...
_PDF_STRINGS: dict[str, dict[str, str]] = {
    ...
    "PL": {
        ...
        "kaava_tuuli":     ("<b>Status planistyczny i wymóg OOŚ:</b> Projekt farmy wiatrowej niemal zawsze "
                            "wymaga planu miejscowego (Ustawa o planowaniu i zagospodarowaniu przestrzennym, "
                            "Dz.U. 2003 nr 80 poz. 717). Procedura OOŚ (Ustawa OOŚ, Dz.U. 2008 nr 199 "
                            "poz. 1227) jest obowiązkowa dla projektów "
        ...
        "kaava_SMR":       ("<b>Wstępne licencjonowanie PAA (najważniejszy pierwszy krok):</b> W przypadku "
                            "obiektu jądrowego decyzja zasadnicza Ministra Klimatu i Środowiska (Prawo "
                            "atomowe) oraz zezwolenie na budowę obiektu jądrowego wydawane przez Prezesa "
                            "Państwowej Agencji Atomistyki (PAA) są obowiązkowe przed wszystkimi innymi "
                            "zezwoleniami. Raport bezpieczeństwa przedkładany PAA inicjuje proces "
                            "licencjonowania. Planowanie odbywa się równolegle, ale procedura "
                            "bezpieczeństwa jądrowego jest czynnikiem dominującym."),
        "kaava_aurinkovoima": ("<b>Pozwolenie na roboty budowlane lub pozwolenie na budowę — i planowanie:</b> "
                            ...
                            "Dz.U. 1994 nr 89 poz. 414). OOŚ nie jest wymagana dla projektów poniżej "
    ...
    "LT": {
        ...
        "kaava_tuuli":     ("<b>Teritorijų planavimo statusas ir PAV reikalavimas:</b> Vėjo "
                            ...
                            "detalusis planas (Teritorijų planavimo įstatymas). PAV procedūra (PAV "
                            "įstatymas) yra privaloma ≥10 MW "
        ...
        "kaava_SMR":       ("<b>Branduolinės energetikos reguliavimas (svarbiausia pastaba):</b> "
                            "Lietuvoje šiuo metu nėra veikiančios atominės elektrinės (Ignalinos AE "
                            "eksploatacija nutraukta 2009 m.). Branduolinės energijos įstatymas (1996, "
                            "su pakeitimais) galioja, o VATESI (Valstybinė atominės energetikos saugos "
                            "inspekcija) yra veikianti priežiūros institucija, tačiau šiuo metu jos "
                            "veikla apima tik Ignalinos AE eksploatavimo nutraukimo ir radioaktyviųjų "
                            "atliekų priežiūrą. Naujo branduolinio objekto statybai reikėtų naujo "
                            "teisinio pagrindo — aiškaus leidimų išdavimo proceso šiuo metu nėra."),
        "kaava_aurinkovoima": ("<b>Statybos leidimas ir teritorijų planavimas:</b> Nedideliam saulės "
                            ...
                            "procedūra pagal Statybos įstatymą. PAV nereikalaujama "
